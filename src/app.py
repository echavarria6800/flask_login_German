...
csrf = CSRFProtect()
db = MySQL(app) #Conexión a base de datos tras importar el módulo
...
def login():
    if request.method == 'POST': #Si lo requerimos por medio de POST si lo enviamos
        ...
        user = User(0, request.form['username'], request.form['password'])#Obtenemos el name de los campos HTML
